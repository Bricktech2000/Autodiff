...
// intended for `move...` parameters. `MOVE` transfers ownership of its tensor
// to the callee while `REF` lends it. all returned tensors are owned
#define MOVE true,
#define REF false,

#define ROW_TENSOR(TENSOR)                                                     \
  tensor_reshape((shape_t){1, (TENSOR)->shape[0]}, MOVE TENSOR)
#define COL_TENSOR(TENSOR)                                                     \
  tensor_reshape((shape_t){(TENSOR)->shape[0], 1}, MOVE TENSOR)
